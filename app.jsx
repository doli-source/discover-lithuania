...
function App() {
  ...
  const effectivePlaces = useMemo(() => {
    return PLACES.map(p => ({ ...p, ...(adminOverrides[p.id] || {}) })).filter(p => p.status !== 'hidden');
  }, [adminOverrides]);

  const [openPlaceId, setOpenPlaceId] = useState(() => parseURL().openPlaceId);

  useEffect(() => { localStorage.setItem('lt_saved', JSON.stringify([...savedSet])); }, [savedSet]);
  ...
  useEffect(() => {
    ...
    let description = lang === 'he' ? 'מדריך טיולים אישי לליטא — 141 מקומות נבחרים' : "A curated travel guide to Lithuania — 141 handpicked places. By Niv Shimoni.";
    ...
    } else if (screen === 'explore') { title = lang === 'he' ? `כל האזורים | ${baseTitle}` : `Explore all regions | ${baseTitle}`;
    } else if (screen === 'routes') { title = lang === 'he' ? `מסלולים | ${baseTitle}` : `Routes | ${baseTitle}`;
    } else if (screen === 'food')   { title = lang === 'he' ? `איפה לאכול | ${baseTitle}` : `Food & drink | ${baseTitle}`;
    } else if (screen === 'stays')  { title = lang === 'he' ? `איפה לישון | ${baseTitle}` : `Where to sleep | ${baseTitle}`; }
    document.title = title;
    const sm = (s,a,v) => { const el=document.querySelector(s); if(el) el.setAttribute(a,v); };
  ...
  useEffect(() => {
    const onPop = () => { const u=parseURL(); setScreen(u.screen); setParams(u.params); setOpenPlaceId(u.openPlaceId); setLangState(u.lang); };
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--density', { compact:'0.85', comfortable:'1', spacious:'1.2' }[tweaks.density] || '1');
    root.style.setProperty('--radius', { sharp:'2px', soft:'12px', round:'20px' }[tweaks.rounded] || '12px');
  ...
  const openPlace = (id) => {
    setOpenPlaceId(id);
    const p = new URLSearchParams(location.search); p.set('place', id);
    history.pushState(null, '', '?' + p.toString());
  };
  const closePlace = () => {
    setOpenPlaceId(null);
    const p = new URLSearchParams(location.search); p.delete('place');
    const qs = p.toString();
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        {screen === 'home' && <HomeScreen lang={lang} t={t} regions={REGIONS} places={effectivePlaces} landmarks={LANDMARKS} dishes={DISHES} itineraries={ITINERARIES} facts={FACTS} mapUrl={MAP_URL} nav={nav} savedSet={savedSet} toggleSaved={toggleSaved} openPlace={openPlace} />}
        {screen === 'explore' && <ExploreScreen lang={lang} t={t} regions={REGIONS} places={effectivePlaces} mapUrl={MAP_URL} params={params} nav={nav} savedSet={savedSet} toggleSaved={toggleSaved} openPlace={openPlace} />}
        {screen === 'routes' && <RoutesScreen lang={lang} t={t} regions={REGIONS} places={effectivePlaces} landmarks={LANDMARKS} itineraries={ITINERARIES} params={params} openPlace={openPlace} />}
        {screen === 'food' && <FoodScreen lang={lang} t={t} places={effectivePlaces} dishes={DISHES} regions={REGIONS} savedSet={savedSet} toggleSaved={toggleSaved} openPlace={openPlace} />}
        {screen === 'stays' && <StaysScreen lang={lang} t={t} places={effectivePlaces} regions={REGIONS} savedSet={savedSet} toggleSaved={toggleSaved} openPlace={openPlace} />}
        {screen === 'saved' && <SavedScreen savedSet={savedSet} places={effectivePlaces} regions={REGIONS} lang={lang} t={t} openPlace={openPlace} toggleSaved={toggleSaved} />}
      </main>
      <Footer lang={lang} t={t} nav={nav} />
      {openPlaceData && <PlaceModal place={openPlaceData} region={openPlaceRegion} lang={lang} t={t} onClose={closePlace} saved={savedSet.has(openPlaceId)} onToggleSaved={toggleSaved} mapUrl={MAP_URL} />}
      <Tweaks lang={lang} tweaks={tweaks} setTweak={setTweak} />
      <a className="wa-float" href={`https://wa.me/972534228282?text=${encodeURIComponent(lang==='en'?'Hey Niv! I have a question about Lithuania 🌿':'היי ניב! יש לי שאלה על ליטא 🌿')}`} target="_blank" rel="noopener noreferrer" aria-label="צור קשר בוואטסאפ">
        <svg viewBox="0 0 24 24" fill="white" width="28" height="28" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.124.558 4.118 1.535 5.845L.057 23.25c-.09.334.232.635.563.52l5.565-1.935A11.942 11.942 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.75c-1.9 0-3.68-.512-5.21-1.402l-.374-.218-3.878 1.348 1.306-3.77-.24-.39A9.711 9.711 0 012.25 12C2.25 6.615 6.615 2.25 12 2.25S21.75 6.615 21.75 12 17.385 21.75 12 21.75z"/></svg>
      </a>
    </div>
  );
}
    ...
    <header className={`nav ${scrolled ? 'scrolled' : ''}`}>
      <div className="nav-inner">
        ...
        <button className="brand" onClick={() => nav('home')}>
          <span className="brand-mark"><svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M12 2 L14 9 L21 11 L14 13 L12 22 L10 13 L3 11 L10 9 Z" opacity="0.95"/></svg></span>
          <span className="brand-text"><span className="brand-name">{t.siteName}</span><span className="brand-sub">{t.siteSub}</span></span>
        </button>
        <nav className="nav-links">
          {items.map(item => (
            <button key={item.id} className={`nav-link ${screen===item.id?'active':''}`} onClick={() => nav(item.id)}>{item.label}</button>
          ))}
        </nav>
        <div className="nav-actions">
          {savedCount > 0 && <button className="saved-btn" onClick={onSavedClick} aria-label={t.yourTrip}><Icon.bookmarkFill/><span>{savedCount}</span></button>}
          <button className="lang-toggle" onClick={() => setLang(lang==='he'?'en':'he')} aria-label="Toggle language">
            <Icon.globe/><span className={lang==='he'?'active':''}>עב</span><span className="lang-sep">·</span><span className={lang==='en'?'active':''}>EN</span>
          </button>
        </div>
      </div>
      {mobileOpen && (
        <div className="mobile-nav">
          {items.map(item => (
            <button key={item.id} className={`mobile-link ${screen===item.id?'active':''}`} onClick={() => { nav(item.id); setMobileOpen(false); }}>{item.label}</button>
          ))}
        </div>
      )}
    </header>
    ...
    <footer className="footer">
      <div className="footer-inner">
        <div className="footer-brand">
          ...
          <p className="footer-sub">{lang==='he'?'מדריך אישי לליטא — בלי פאתוס, רק מה שכיף.':"A friendly guide to Lithuania — no pretense, just what's fun."}</p>
          <div className="footer-about">
            <p className="footer-about-bio">{lang==='he'?'ניב שמעוני. יזם, עובד עם AI, ומבלה בליטא יותר ממה שתכנן. האתר הזה הוא התוצאה.':'Niv Shimoni. Entrepreneur, AI builder, and someone who keeps ending up in Lithuania. This site is the result.'}</p>
        ...
        <div className="footer-cols">
          ...
          <div className="footer-col">
            ...
            {REGIONS.map(r => <button key={r.id} onClick={() => nav('explore', { region: r.id })}>{r[lang].name}</button>)}
          </div>
          <div className="footer-col">
  ...
  return (
    <TweaksPanel title="Tweaks">
      <TweakSection label={lang==='he'?'צבע נושא':'Theme color'}>
        <TweakColor label={lang==='he'?'פלטה':'Palette'} value={PALETTES[tweaks.palette]?.primary||PALETTES.amber.primary}
          options={['amber','forest','baltic','brick'].map(k=>PALETTES[k].primary)}
          onChange={(v)=>{ const f=Object.entries(PALETTES).find(([_,p])=>p.primary===v); if(f) setTweak('palette',f[0]); }}/>
      </TweakSection>
      <TweakSection label={lang==='he'?'טיפוגרפיה':'Typography'}>
        <TweakSelect label={lang==='he'?'פונטים':'Font pair'} value={tweaks.fontPair}
          options={Object.entries(FONT_PAIRS).map(([k,v])=>({value:k,label:v.name[lang]}))} onChange={(v)=>setTweak('fontPair',v)}/>
      </TweakSection>
      <TweakSection label={lang==='he'?'תצוגה':'Display'}>
        <TweakRadio label={lang==='he'?'צפיפות':'Density'} value={tweaks.density}
          options={[{value:'compact',label:lang==='he'?'דחוס':'Compact'},{value:'comfortable',label:lang==='he'?'נוח':'Cozy'},{value:'spacious',label:lang==='he'?'מרווח':'Roomy'}]}
          onChange={(v)=>setTweak('density',v)}/>
        <TweakRadio label={lang==='he'?'פינות':'Corners'} value={tweaks.rounded}
          options={[{value:'sharp',label:lang==='he'?'חדות':'Sharp'},{value:'soft',label:lang==='he'?'רכות':'Soft'},{value:'round',label:lang==='he'?'עגולות':'Round'}]}
